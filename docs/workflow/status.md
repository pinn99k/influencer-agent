# Workflow Status
project: 인플루언서 에이전트
updated: 2026-06-21 (세션 39 — MVP 마감: 버그수정 + 공개 레포 준비)

## current_step: 8 -> 2장 진입 — 1장(워크플로우 MVP) 졸업 선언, 2장(자율 에이전트 하네스) 시작
## 2장 진행: Phase A(디렉티브)+Phase C(도구 호출 루프) 완료 -> 세션 38에서 자율루프를 CLI(--autonomous)·웹(mode 토글)에 연결(제품화). 선형 run() 기본·폴백 병존

## 세션 39 (2026-06-21) — MVP 마감 + 공개 레포 준비
실 e2e 점검 -> 사용성 버그 수정 -> MVP 3수정 마감 -> 공개 레포(influencer-agent) 동기화.
- 웹 복구 버그 2건(핑구 발견): MD_CACHE 교차오염(파일명만 키잉->대상자명+파일명+clearMdCache 무효화),
  매니저 알림 디스크 복원(load_manager_outputs + GET /api/manager/{name} + openSubject/bootRecover).
- 실 LLM e2e: 자율1회(방향/provenance/stop_reason), 피드백 재분석(컨셉기획 타게팅+촬영팁 반영),
  4주 성과 시뮬(주차별 누적 재분석 4회): W1 보수적->W2 전면개편(일상0/비포애프터+저녁)->W3 수렴(재실행없음)->W4 갱신.
- MVP 마감 3수정: ConnectionError 재시도(llm_client), [추정] 가드(장비/성공지표),
  _synthesize_final_report 페이로드에 성과_기록 주입+실제 달성치 반영(팔로워330/저장180/예약문의, [추정]제거 검증).
- 공개 준비: outputs 전체 삭제, 비-격리 테스트 2개(test_call_llm_tools/test_ceo_decide_next) 더미키 -> 키독립.
  클론 src 동기화(.env/outputs/캐시 제외)+README 392+커밋 63888af. 클론 키없이 391pass+1skip. push 대기(URL).
- ADR 035, review 2026-06-21-세션39.

### 다음 시작점
- 공개 레포 push(원격 URL+핑구 확인). 실제 미용사 섭외 -> 4주 운영. cosmetic: 페르소나 칩 잔상.

## 세션 38 (2026-06-16) — 2장 자율루프 제품 연결
ADR 031의 유일한 미해결 항목 해소: 도구 호출 루프(run_autonomous)가 제품 어디서도
호출되지 않던(테스트 전용) 상태 -> CLI·웹에 연결. 선형 run()은 기본/폴백 병존 유지.
- CLI: main.py --autonomous 플래그(autonomous and not dry_run -> run_autonomous).
- 웹: JobContext.mode + start_job(mode=) + _run_ceo 분기 + job_completed에 mode.
  /start, /interview/confirm 라우트가 mode 수용·검증(둘 외엔 linear 폴백)·전달.
- 프론트: 인터뷰 확정 바 '자율 모드' 토글(State.runMode) -> interviewConfirm mode 전달.
  확정 바 세로 스택 레이아웃. 테스트 위생: win32 stdout 재래핑을 pytest 하 스킵.
- 검증: 379 pass(373->379, 신규 test_autonomous_wiring 6), 한자 0, node --check 4 OK,
  브라우저 스모크(상태 주입·LLM 비용 없음) 토글 렌더/클릭/재렌더/콘솔0/레이아웃.
- 자체 코드리뷰(실행 없이, 호출/응답 흐름 점검) -> 결함 F1 발견·수정: run_autonomous가
  run()과 달리 _load_direction/성과 주입을 건너뛰어 자율 모드에서 사용자 방향이 누락되던 문제.
  ceo.py run_autonomous에 성과+_load_direction 주입(run()과 동일). 회귀 가드 테스트 추가.
  (완료메시지/대시보드 plan/데모경로는 트레이싱 결과 무회귀로 확인.)
- T0 위생: State.runMode를 actions.js reset에서 'linear' 초기화(자율 토글 끈적임 제거).
- T1 자율 루프 관찰성·provenance: ToolExecutor가 각 도구호출을 MeasureStore에 AI 결정으로
  기록(focus->basis, 워커->decision, 검증미통과 표시) + AgentLoop가 stop_reason
  (finish/no_tool/max_iter) 반환·loop_stopped emit -> run_autonomous pipeline_completed에 노출.
- 검증(누적): 386 pass(373->386, 신규 wiring6+direction1+observability6), 한자 0,
  node --check OK, 임시 헬퍼 0. 실 LLM e2e는 미실행(핑구 지시 — 다음 세션).
- 다음 세션(e2e 포함):
  (1) 자율 1회 실 gpt-4o 실행 -> 방향 실반영 + provenance 로그(decision_log) + stop_reason 육안 확인
  (2) T2: 워크플로우 vs 에이전트 A/B 비교 하네스 빌드(고정 성과 스냅샷 fixture + 비교 러너 골격 +
      측정 수집기, mock 테스트). 실제 비교 실행은 실 gpt-4o(e2e).
- 잔존물: src/_e2e_*.txt(이전 세션 보존본, 핑구 확인 후 삭제). 커밋 없음(git 미지시).
...
- [ADR 034] docs/adr/034-자율루프-제품연결.md (세션 38)
- [ADR 035] docs/adr/035-mvp마감-버그수정-공개레포.md (세션 39)
...
- [2026-06-16] docs/review/2026-06-16-세션38.md
- [2026-06-21] docs/review/2026-06-21-세션39.md
