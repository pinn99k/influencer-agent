...
> 또한 이 시스템은 도구를 스스로 호출하는 "자율 에이전트"가 아니라,
> 정해진 흐름을 도는 **잘 구조화된 워크플로우**에 가깝다(정체성 정정은 ADR 029 참조).
...
설계 문서: `docs/workflow/`(단계별) · `docs/adr/`(결정 30건) · `docs/review/`(세션 회고)
...
| 최소 자율 관리 루프 | 부분 구현 (도구 호출 루프 프리미티브 — CEO.run_autonomous) |
| 다부서 가로 확장 / Cross-review | 로드맵 |

> 개발 방식: 문제 정의·설계·기술 결정은 직접 하고, 코드는 AI 협업으로 작성했으며,
> 주요 결정은 ADR 30건으로 근거를 남겼다.
